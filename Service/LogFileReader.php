...
class LogFileReader
{
    ...
    public function readGrep(string $fileName, string $pattern, int $limit = 500, bool $fullScan = false): string
    {
        ...
        if ($fullScan) {
            $cmd = "grep -a -- {$pattern} {$path} | tail -n {$limit} 2>/dev/null";
        } else {
            // Scan last 50 MB only — orders of magnitude faster on large files
            $cmd = "tail -c 52428800 {$path} | grep -a -- {$pattern} | tail -n {$limit} 2>/dev/null";
        }
    ...
    public function aggregateExceptions(string $fileName, int $scanLines = 10000): array
    {
        ...
        foreach (explode("\n", $raw) as $line) {
            ...
            if (!preg_match(
                '/\[object\]\s*\((\w+(?:Exception|Error|Throwable))'
                . '(?:\(code:[^)]*\))?:\s*(.{0,200}?)'
                . '(?:\s+at\s+\S+|\s+in\s+\S+|\\\\n|$)/u',
                $line,
                $em
                ...
                if (!preg_match(
                    '/\.(?:ERROR|CRITICAL).*?(\w+(?:Exception|Error|Throwable))'
                    . '(?:\(code:[^)]*\))?[:\s]+(.{0,160}?)(?:\s*\{|$)/u',
                    $line,
                    $em
                )) {
