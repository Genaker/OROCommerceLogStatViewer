...
class ServerMetricsCollectorTest extends TestCase
{
    ...
    public function testCollectReturnsRequiredTopLevelKeys(): void
    {
        ...
        $requiredKeys = ['instanceId', 'hostname', 'ip', 'collectedAt', 'uptime', 'load', 'memory', 'cpu', 'disk', 'processes'];
    ...
    /** @test */
    public function testCpuUsagePctIsWithinRange(): void
    {
        $snapshot = $this->collector->collect();

        $this->assertArrayHasKey('usagePct', $snapshot['cpu']);
        $this->assertIsFloat($snapshot['cpu']['usagePct']);
        $this->assertGreaterThanOrEqual(0.0, $snapshot['cpu']['usagePct']);
        $this->assertLessThanOrEqual(100.0, $snapshot['cpu']['usagePct']);
    }

    /** @test */
    public function testDiskContainsRequiredKeys(): void
    {
        $snapshot = $this->collector->collect();

        foreach (['total', 'free', 'used', 'usedPct'] as $key) {
            $this->assertArrayHasKey($key, $snapshot['disk'], "Missing disk key: $key");
        }
    }

    /** @test */
    public function testDiskUsedPctIsBetweenZeroAndOneHundred(): void
    {
        $snapshot = $this->collector->collect();
        $pct      = $snapshot['disk']['usedPct'];
    ...
    /** @test */
    public function testUptimeIsNonEmptyString(): void
    {
        $snapshot = $this->collector->collect();

        $this->assertIsString($snapshot['uptime']);
        $this->assertNotEmpty($snapshot['uptime']);
    }
