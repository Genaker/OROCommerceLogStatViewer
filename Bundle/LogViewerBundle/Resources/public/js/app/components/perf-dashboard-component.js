...
define([
    ...
    const PerfDashboardComponent = BaseComponent.extend({
        ...
        _renderCard: function(inst) {
            ...
            const cpu = inst.cpu || {};
            const disk = inst.disk || {};
            const procs = inst.processes || [];
            const uptime = inst.uptime || 'unknown';
            ...
            const diskCls = disk.usedPct >= MEM_CRIT ? 'pd-crit'
                : (disk.usedPct >= MEM_WARN ? 'pd-warn' : 'pd-ok');
            const cpuCls = cpu.usagePct >= 90 ? 'pd-crit'
                : (cpu.usagePct >= 75 ? 'pd-warn' : 'pd-ok');

            const memBar = Math.min(100, mem.usedPct || 0);
            const diskBar = Math.min(100, disk.usedPct || 0);
            ...
            return '<div class="pd-card ' + ageCls + '">' +
                ...
                '<div class="pd-card-header">' +
                    '<span class="pd-card-hostname">\u{1f4bb} ' +
                        inst.hostname + '</span>' +
                    '<span class="pd-card-uptime" title="Uptime">\u23f1 ' + uptime + '</span>' +
                ...
                '<div class="pd-metrics">' +
                    ...
                    '<div class="pd-metric">' +
                        '<span class="pd-metric-label">CPU usage</span>' +
                        '<span class="pd-metric-val ' + cpuCls + '">' +
                            (cpu.usagePct !== undefined ? cpu.usagePct : '—') + '<small>%</small></span>' +
                        '<span class="pd-metric-sub">' + cores + ' core' + (cores !== 1 ? 's' : '') + '</span>' +
                        '<span class="pd-metric-sub">Load: ' + load.m1 + ' / ' + load.m5 + ' / ' + load.m15 +
                            ' <small>(1m/5m/15m)</small></span>' +
                    '</div>' +
                    ...
                    '<div class="pd-metric">' +
                        '<span class="pd-metric-label">Disk usage</span>' +
                        '<span class="pd-metric-val ' + diskCls + '">' +
                            disk.usedPct + '<small>%</small></span>' +
                        '<div class="pd-bar-track"><div class="pd-bar-fill ' +
                            diskCls + '" style="width:' + diskBar + '%"></div></div>' +
                        '<span class="pd-metric-sub">' +
                            this._kbToGb(disk.used) + ' / ' + this._kbToGb(disk.total) +
                            '</span>' +
                    '</div>' +
